...
# ============================================================
# Chimera CLI (NEXUS-OMEGA) �?一键安装脚�?(Windows PowerShell)
#
# 用法:
#   一行命�?PS 5.1 / PS 7+ 均兼�?:
...
#
# 离线/手动下载场景:
#   .\install.ps1 -LocalFile 'C:\Users\$env:USERNAME\Downloads\chimela-windows-x86_64.exe' -Version v2.26.0-omega
#
...
#
#   如果 irm 持续 404,建议直接克隆仓库后本地执�?
#     git clone https://github.com/Yoloccyt/Chimera-CLI-.git
#     cd Chimera-CLI-
#     $env:GITHUB_TOKEN='ghp_xxx'; .\install.ps1
#
# 参数说明:
#   -Version <ver>      指定版本 (默认: latest,�?v2.26.0-omega)
#   -InstallDir <path>  安装目录 (默认: $env:LOCALAPPDATA\Programs\chimela)
#   -Proxy <url>        HTTP/HTTPS 代理地址 (�?http://proxy.company.com:8080)
#   -LocalFile <path>   使用预先下载的本�?binary 安装,跳过所有网络请�?
#   -SkipVerify         跳过 SHA256 校验
#   -SkipVersionCheck   跳过 --version 验证(企业安全策略拦截首次运行等场�?
#   -SetupEnv           仅设置工具链环境变量后退�?不下�?binary
#                       (用于源码构建场景,覆盖 §10.5 "环境变量手动设置"短板)
#
# 功能:
#   - 自动检测架�?(x86_64/aarch64,ARM64 降级 x86_64 兼容�?
#   - �?GitHub Release 下载 chimela-windows-x86_64.exe
#   - 可�?SHA256 校验 (�?Release 附带 checksums.txt)
#   - 安装�?$env:LOCALAPPDATA\Programs\chimela\ (默认)
#   - 同时生成 chimela.exe / aether.exe / chimera.exe 三个入口
#     (chimela 为新品牌�?aether �?cargo 二进制名,chimera 为兼容别�?
#   - 添加到用�?PATH (通过 [Environment]::SetEnvironmentVariable)
...
#
# 退出码:
#   0  安装成功(�?-SetupEnv 模式下环境变量设置成�?
#   1  安装失败(网络/鉴权/校验/权限错误,�?[ERROR] 输出)
#
# �?release.yml 一致�?
...
# 严格模式: 捕获未定义变量、强制类型转换失�?
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
# WHY: PS 5.1 �?Invoke-WebRequest 进度条更新会严重拖慢大文件下�?
#      在企业网络场景下可能导致超时。静默进度条可显著改善下载性能�?
$ProgressPreference = 'SilentlyContinue'

# ------------------ 网络层健壮性初始化 ------------------
# WHY: PowerShell 5.1 默认仅启�?TLS 1.0 / SSL3,�?GitHub 等现�?CDN 要求 TLS 1.2+�?
#      企业 DPI 防火墙对�?TLS 握手进行深度检测并阻断,强制 TLS 1.2 可绕过此类检�?
#      �?Podman 安装经验一�?[Net.SecurityProtocolType]::Tls12 可成功下�?GitHub 资源)�?
...
try {
    ...
    # 仅在成功设置后才输出,避免�?-SetupEnv 等静默模式下产生噪音
    if (-not $SetupEnv) {
        Write-Host "[INFO] 已强制启�?TLS 1.2/1.3 (原始协议: $currentProtocols)" -ForegroundColor Cyan
    }
} catch {
    Write-Warning "无法强制 TLS 1.2/1.3: $($_.Exception.Message)"
}

# 代理支持: 构造统一�?Invoke-WebRequest 通用参数 splat
...
#      此函数将工具�?env 注入用户级环境变�?持久化避免每次新 shell 重复设置�?
#      路径基于 $PSScriptRoot 推导,支持仓库位于任意盘符;MINGW 通过 gcc 探测�?
#      �?Windows 平台适用�?
function Set-Environment {
    <#
    .SYNOPSIS
        自动设置 Chimera CLI 工具链环境变�?CARGO_HOME/RUSTUP_HOME/PATH/TMP/TEMP)
    .DESCRIPTION
        基于脚本所在目录推导项目根目录,�?.toolchain\cargo\bin �?MSYS2 mingw64\bin
        注入用户级环境变�?持久化保存。同时同步当前会�?避免必须重启终端�?
        覆盖 §10.5 已知基建短板"环境变量仍需手动设置"�?
    .EXAMPLE
        .\install.ps1 -SetupEnv
    #>
    # WHY: 仓库可能�?D 盘以外的位置,不能写死路径。优先使�?$PSScriptRoot;
    ...
    # WHY: MSYS2 也可能安装在其他位置,优先�?where.exe gcc 探测真实路径;
    #      探测失败再回退到默�?D:\msys64\mingw64\bin�?
    ...
    if ($gccCommand -and $gccCommand.Source) {
        ...
        if ($detectedMingwBin -and (Test-Path (Join-Path $detectedMingwBin 'gcc.exe'))) {
            ...
            Write-Info "探测�?MINGW: $mingwBin"
        }
    } elseif (-not (Test-Path (Join-Path $mingwBin 'gcc.exe'))) {
        Write-WarnMsg '未找�?gcc.exe,将使用默�?MINGW 路径;若后�?cargo build 失败请检�?MSYS2 安装'
    }

    # 设置 CARGO_HOME / RUSTUP_HOME(用户�?持久�?
    ...
    Write-Ok "已设�?CARGO_HOME=$toolchainCargoHome"
    Write-Ok "已设�?RUSTUP_HOME=$toolchainRustupHome"

    # WHY: 项目构建临时文件默认重定向到项目根目�?tmp,避免 C 盘空间不足或权限问题�?
    ...
    if (Test-Path $tmpDir) {
        ...
        $env:TMP = $tmpDir
        $env:TEMP = $tmpDir
        Write-Ok "已设�?TMP/TEMP=$tmpDir"
    } else {
        Write-WarnMsg "未找到项�?tmp 目录 $tmpDir,跳过 TMP/TEMP 重定�?
    }
    ...
    if ($currentPath -notlike "*$toolchainCargo*") {
        ...
        if ($newPath.Length -gt 8191) {
            ...
            Write-WarnMsg "请手动将以下目录添加�?PATH: $toolchainCargo; $mingwBin"
        } else {
        ...
        Write-WarnMsg "$toolchainCargo 已在 PATH �?
    }

    # ------------------ 工具�?channel 选择(GNU) ------------------
    # WHY: 本项目必须用 GNU 工具�?stable-x86_64-pc-windows-gnu)—�?cargo/config.toml �?
    #      [target.x86_64-pc-windows-gnu] linker="gcc" 仅对 GNU 目标生效。但多数 Windows 机器
    #      rustup �?默认宿主"�?MSVC(�?rustup show �?Default host),仅靠系统默认会选到 MSVC
    #      目标导致链接失败。故在项目本�?RUSTUP_HOME 内显式把默认工具链设�?GNU�?
    # WHY 不用 rust-toolchain.toml: 它是全局单值、按"默认宿主"展开——在 MSVC 宿主�?channel="stable"
    #      会解析成 MSVC;而写�?GNU 三元组又会让 release.yml �?Linux/macOS runner 无法安装�?
    #      Windows 宿主工具链而失败。项目本�?rustup default 既确�?又不污染跨平�?CI�?
    ...
    if (Get-Command rustup -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Ok "已将默认工具链设�?$gnuToolchain"
        } else {
            Write-WarnMsg "设置默认工具链失�?请手动执�? rustup default $gnuToolchain"
        }
    } else {
        Write-WarnMsg "未找�?rustup,跳过 channel 设置;装好 rustup 后请执行: rustup default $gnuToolchain"
    }

    Write-Host ""
    Write-Host "用户级环境变量已持久�?当前会话也已同步�? -ForegroundColor Cyan
    Write-Host "如仍无法识别 cargo/rustup,请重新打开 PowerShell 终端�? -ForegroundColor Cyan
}

# ------------------ 配置常量 ------------------
# InstallDir 默认值需要延迟到运行时计�?(依赖 $env:LOCALAPPDATA)
...
# -SetupEnv 短路: 仅设置环境变量后立即退�?不进入下载安装流�?
# WHY: 必须放在 Write-Ok/Write-WarnMsg 等函数定义之�?确保 PowerShell 7 �?
#      顺序执行模式下函数已可用�?
if ($SetupEnv) {
    Set-Environment
    exit 0
...
if (-not [System.Environment]::Is64BitOperatingSystem) {
    Die '不支�?32 位操作系�?(仅支�?x86_64 / aarch64)'
}

# 检测处理器架构 (AMD64 �?x86_64,ARM64 �?aarch64)
...
if ($processorArch -match 'AMD64|X64') {
    ...
    Die '不支�?32 �?ARM (仅支�?x86_64 / aarch64)'
} else {
    Die "不支持的架构: $processorArch (仅支�?x86_64 / aarch64)"
}

# Windows 当前 Release 仅有 x86_64 binary (release.yml matrix)
# Windows 11 ARM 可通过 x86_64 兼容层运�?
if ($archNorm -ne 'x86_64') {
    Write-WarnMsg "检测到架构 $archNorm,但当�?Release 仅有 Windows x86_64 binary"
    Write-WarnMsg '将使�?x86_64 binary (Windows 11 ARM 可通过 x86_64 兼容层运�?'
    $archNorm = 'x86_64'
}
...
# ------------------ 本地文件模式短路 ------------------
# WHY: 企业网络常阻�?GitHub releases 直连,允许用户预先�?binary 下载到本�?
...
# ------------------ 版本号归一�?------------------
# WHY: 用户常输�?1.5.8-omega 而非 v2.26.0-omega,直接拼入 Release URL 会导�?404�?
#      此处自动�?v 前缀,同时保留 latest/local 等特殊标�?避免无意义失败�?
...
if (-not $localFileMode -and [string]::IsNullOrEmpty($Version)) {
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($Version)) {
            Die '无法解析最新版本号 (仓库可能未发�?Release)'
        }
        Write-Info "最新版�? $Version"
    } catch {
        Die "无法访问 GitHub API: $($_.Exception.Message)
可能原因与解决方�?
  1) 网络连接问题 �?检查能否访�?https://api.github.com
  2) 企业防火�?DPI 阻断 �?使用 -Proxy 参数,或先手动下载后用 -LocalFile 安装
     示例: .\install.ps1 -Proxy 'http://proxy.company.com:8080'
     示例: .\install.ps1 -LocalFile 'C:\Users\$env:USERNAME\Downloads\chimela-windows-x86_64.exe' -Version v2.26.0-omega
  3) 仓库为私�?(需设置 `$env:GITHUB_TOKEN)
  4) GitHub API 速率限制"
    }
...
# ------------------ 下载辅助函数 ------------------
# WHY: 企业网络/DPI 防火墙常见阻断策略是在数据传输阶段中断连�?单次重试不足�?
#      此函数提供三层降级策�?
#        1. Invoke-WebRequest + 重试(默认,支持代理)
#        2. curl.exe (Win10+ 1803 内置,绕开 PS 5.1 WebRequest 限制)
#        3. Start-BitsTransfer (支持断点续传,适合不稳定网�?
#      每层失败后自�?fallback 到下一�?确保尽可能完成下载�?
function Invoke-DownloadWithRetry {
    ...
    for ($attempt = 1; $attempt -le $MaxRetries; $attempt++) {
        try {
            ...
            if ($attempt -lt $MaxRetries) {
                ...
                Write-WarnMsg "下载�?$attempt/$MaxRetries 次失�?${waitSec}s 后重�? $($_.Exception.Message)"
                Start-Sleep -Seconds $waitSec
            } else {
                Write-WarnMsg "下载�?$attempt/$MaxRetries 次失�? $($_.Exception.Message)"
            }
        }
    }
    ...
    # ---- 3. Start-BitsTransfer 回退 (支持断点续传,适合不稳定网�? ----
    # WHY: BITS 后台智能传输可自动恢复网络中�?�?Windows 11 默认禁用�?
    #      不作为首�?仅作�?Invoke-WebRequest �?curl 均失败时的最后尝试�?
    try {
...
try {
    ...
    if ($localFileMode) {
        # WHY: 本地文件模式完全跳过网络,直接复制到临时目录参与后续安装流�?
        ...
        Write-Ok "本地 binary 已复�? $fileSizeMB MB"
    } else {
        # ------------------ 下载 binary ------------------
...
所有下载方式均失败,可能原因:
  ...
  3) 网络连接问题 / 企业 DPI 阻断
    ...
    # ------------------ SHA256 校验 (可�? ------------------
    # WHY: 本地文件模式下网络不可达,跳过 checksum 下载(由用户自行确保本�?binary 来源可信)
    if (-not $SkipVerify -and -not $localFileMode) {
        ...
        try {
            ...
            Write-WarnMsg 'Release 未附�?checksums.txt 或网络不可达,跳过 SHA256 校验'
        }
        ...
        if ($checksumAvailable) {
            ...
            foreach ($line in ($checksumContent -split "`n")) {
                # 匹配 "HASH  filename" �?"HASH *filename"
  ...
  实际: $actualHash"
    ...
    #        - aether.exe:  cargo 内部二进制名(开发兼�?
    #      Windows 符号链接需要特殊权限且兼容性差,直接复制更可�?仅约 1.3MB)�?
    ...
    # WHY: 从网络下载的 exe 会携�?Internet 区域标记(MOTW),可能触发 Windows Defender/
    #      SmartScreen/企业 AV 的拦�?导致 --version 验证失败。安装后解除标记�?
    ...
    Write-Ok 'binary 已安�?(chimera.exe + chimela.exe + aether.exe 兼容别名)'
    ...
    # 检�?InstallDir 是否已在用户 PATH �?(大小写不敏感)
    $alreadyInPath = $false
    ...
    if (-not $alreadyInPath) {
        try {
            ...
            # WHY: Windows 用户�?PATH 过长会导致注册表写入被截断或后续进程解析失败�?
            #      8191 是现�?Windows API 的安全上�?超过时拒绝自动写入并提示手动添加�?
            if ($newPath.Length -gt 8191) {
                Write-WarnMsg "用户 PATH 过长 ($($newPath.Length) 字符),超过 8191 安全上限"
                Write-WarnMsg "请手动将 $InstallDir 添加�?PATH"
            } else {
                [Environment]::SetEnvironmentVariable('Path', $newPath, 'User')
                $pathUpdated = $true
                Write-Info 'PATH 已更�?(用户�?'
                # 当前会话也立即生�?
    ...
    if ($pathUpdated) {
        Write-WarnMsg '请重�?PowerShell 终端以使 PATH 全局生效'
    }
    ...
    foreach ($exePath in @($installPath, $chimelaPath, $aetherPath)) {
        try {
            ...
            if ($matched) {
                ...
                Write-WarnMsg "可能原因: 缺少 VC++ 运行�?/ Windows Defender 拦截 / 文件损坏"
                Write-WarnMsg "请手动执�? $exePath --version"
            }
        } catch {
            $versionFailed++
            Write-WarnMsg "$exePath --version 执行失败: $($_.Exception.Message)"
            Write-WarnMsg '可能缺少运行时依�?(Visual C++ Redistributable)'
        }
    }

    if ($versionFailed -gt 0 -and -not $SkipVersionCheck) {
        Die "安装验证失败 ($versionFailed/3 个入口不可用)。请检查上述警�?或使�?-SkipVersionCheck 跳过验证�?
    }
    ...
    Write-Info '=========================================='
    Write-Host ''
    Write-Ok "执行 'chimela --help'�?aether --help' �?'chimera --help' 开始使�?
