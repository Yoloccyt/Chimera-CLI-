...
#
# 离线/手动下载场景:
#   .\install.ps1 -LocalFile 'C:\Users\$env:USERNAME\Downloads\chimela-windows-x86_64.exe' -Version v2.27.1-omega
#
...
#
# 参数说明:
#   -Version <ver>      指定版本 (默认: latest,如 v2.27.1-omega)
...
# ------------------ 版本号归一化 ------------------
# WHY: 用户常输入 1.5.8-omega 而非 v2.27.1-omega,直接拼入 Release URL 会导致 404。
...
可能原因与解决方案:
  ...
  2) 企业防火墙/DPI 阻断 → 使用 -Proxy 参数,或先手动下载后用 -LocalFile 安装
     ...
     示例: .\install.ps1 -LocalFile 'C:\Users\$env:USERNAME\Downloads\chimela-windows-x86_64.exe' -Version v2.27.1-omega
