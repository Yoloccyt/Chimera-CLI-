...
function Invoke-RuleChecks {
    ...
    # WHY: 2.2 requires the L10 *interface* layer to obtain cross-layer data
    # exclusively via EventBus. Before PS-2 every violating edge was *downward*
    # (chimera-tui -> L5..L9), which Check B (upward-only) never caught; the
    # documented rule therefore had no CI enforcement at all. PS-2 batches 1-3
    # removed all 5 such edges (parliament / gqep-executor / pvl-layer /
    # gsoe-evolution / chimera-mas); this check keeps them from coming back.
    #
    # SCOPE = chimera-tui ONLY (not every L10 crate): the other L10 crates
    # (chimera-cli, nexus-app-server, chtc-bridge, mca-gateway, mcp-mesh) are
    # composition roots / host facades whose JOB is to instantiate lower-layer
    # components (e.g. nexus-app-server -> nexus-subagent L7 in
    # subagent_engine.rs). Forbidding their downward edges would forbid
    # wire-up itself. If another *pure consumer* L10 crate appears, add it here.
    $l10InterfaceCrates = @('chimera-tui')
    foreach ($crate in $l10InterfaceCrates) {
        if (-not $layerMap.ContainsKey($crate)) { continue }
        if (-not $DepGraph.ContainsKey($crate)) { continue }
        foreach ($dep in @($DepGraph[$crate])) {
            if ($null -eq $dep) { continue }  # empty graph slot
            $key = "$crate,$dep"
            if ($adrExceptions.ContainsKey($key)) { continue }  # ADR-exempted edge
            if (-not $layerMap.ContainsKey($dep)) { continue }  # external dep
            $depLayer = $layerMap[$dep]
            if ($depLayer -ge 2 -and $depLayer -le 9) {
                $script:report += "[GAP-E] $crate (L10) -> $dep (L$depLayer) outbound violation; route cross-layer data via EventBus (2.2)"
                $script:status = 1
            }
        }
    }
...
if ($SelfTest) {
    ...
    $mockGraph = @{
        ...
        'nexus-contracts' = @()
        'chimera-tui'     = @('nexus-contracts', 'event-bus', 'pvl-layer')  # GAP-E (L10 -> L7)
    }
