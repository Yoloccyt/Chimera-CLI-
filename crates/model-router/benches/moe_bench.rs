//! MoE 稀疏门控路由延迟基准 — 对比 O(n) 全量评估 vs O(k) Top-K 门控
//!
//! 对应 Task 3 [I1]:验证 50+ 模型规模下 MoE 门控的路由延迟收益。
//!
//! # 基准项
//! - `full_O(n)`:退化门控(threshold=MAX)→ route_auto 全量归一化评估
//!   + n-1 候选列表生成(模拟未启用 MoE 的基线)
//! - `moe_O(k)`:门控门控(threshold=50)→ Top-5 粗筛 + 仅对 5 候选完整评估
//!
//! # 规模
...
/// MoE 路由延迟基准:对比全量评估与 Top-K 门控
///
/// WHY 两个 gate:
/// - `full_gate`(threshold=MAX):永不触发门控,走全量评估路径(基线)
/// - `moe_gate`(threshold=50):50+ 规模触发门控,走 Top-K 路径(优化)
///
/// 两者调用同一 `route_auto_with_gate`,仅 gate 配置不同,确保对比公平
...
fn route_latency_bench(c: &mut Criterion) {
    ...
    for n in [50usize, 100, 200] {
        ...
        group.bench_with_input(BenchmarkId::new("moe_O(k)", n), &n, |b, _| {
            b.iter(|| {
                let _decision = model_router::strategies::route_auto_with_gate(
                    black_box(&registry),
                    black_box(&req),
