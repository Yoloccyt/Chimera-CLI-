...
/// LSH 查询探针数 — 每表探测桶数(1 = 仅基础桶,2 = 基础桶+汉明距离1)
const LSH_NUM_PROBES: usize = 2;
...
impl SemanticMemory {
    ...
    pub fn insert(&self, mut entry: MemoryEntry) -> Result<Option<MemoryEntry>, MlcError> {
        ...
        if inner.entries.contains_key(&entry.id) {
            // 找到旧向量索引并从 LSH 索引中移除
            if let Some(old_idx) = inner.vectors.iter().position(|(_, id)| id == &entry.id) {
                let old_vector_idx = old_idx;
                let (old_clv, _) = inner.vectors.get(old_idx).unwrap();
                if inner.lsh_index.is_enabled() {
                    inner.lsh_index.remove(old_vector_idx, old_clv.as_slice());
                }
            }
        ...
        // 添加新向量
        inner.vectors.push_back((shared_clv.clone(), entry.id.clone()));
        // 插入主存储
        inner.entries.insert(entry.id.clone(), Arc::new(entry));

        // 插入到 LSH 索引(P0-3)
        inner.lsh_index.insert(new_vector_idx, shared_clv.as_slice());
    ...
    fn evict_oldest_locked(
        ...
        // 从 LSH 索引中移除(P0-3)
        // 优化:使用rebuild方法替代逐条移除,避免O(n²)开销
        // 当驱逐导致大量索引偏移时,重建比逐条更新更高效
        if inner.lsh_index.is_enabled() {
            // 重建LSH索引:使用当前vectors的剩余元素
            let vectors_to_rebuild: Vec<(SharedCLV, usize)> = inner
                .vectors
                .iter()
                .enumerate()
                .map(|(idx, (clv, _))| (clv.clone(), idx))
                .collect();
            inner.lsh_index.rebuild(&vectors_to_rebuild);
        }
    ...
    pub fn remove(&self, id: &str) -> Result<Option<MemoryEntry>, MlcError> {
        ...
        if entry.is_some() {
            ...
            // P0-3:重建 LSH 索引(移除后索引位置变化)
            if inner.lsh_index.is_enabled() {
                inner.lsh_index.clear();
                for (idx, (clv, _)) in inner.vectors.iter().enumerate() {
...
mod tests {
    ...
    fn test_lsh_not_enabled_below_threshold() {
        ...
        for i in 0..999 {
            mem.insert(make_entry(&format!("m-{i}"), clv.clone())).unwrap();
        }
    ...
    fn test_lsh_memory_overhead() {
        ...
        assert!(lsh_mem < 10_000_000, "LSH 索引内存应 < 10MB,实际为 {lsh_mem}");
    }
    ...
    fn test_lsh_and_linear_scan_equivalent() {
        ...
        for i in 0..500 {
            ...
            mem.insert(make_entry(&format!("m-{i}"), entry_clv)).unwrap();
        }
