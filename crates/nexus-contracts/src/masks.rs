...
//! # 设计决策(WHY)
//!
//! - **从 osa-coordinator 上提**: 原 `SparseMask<T>` 定义在 `osa-coordinator/src/masks.rs`，
//!   被 `OmniSparseMasks` 引用。上提至 L0 后消除 L6 Router × 3 对 OSA 的星型依赖
//!
//! - **泛型 T: Clone + PartialEq + Eq + Hash**: 支持 ToolId/FileId/MemoryId/OperationId/TaskId
//!   五种 ID 类型复用同一容器，Eq + Hash 约束使 `is_active` 可用 HashSet 实现 O(1) 查找
//!
//! - **active_ids: `Vec<T>`**: 保留活跃 ID 列表(序列化字段)，稀疏化后仅包含被选中的项
//!
//! - **active_set: `HashSet<T>`**: `active_ids` 的 HashSet 镜像(`#[serde(skip)]`)，
//!   `is_active` O(1) 查找。反序列化时从 `active_ids` 重建，保持向后兼容
//!
//! - **sparsity_ratio: f32**: 稀疏度 [0.0, 1.0]，1.0 表示全稀疏(无活跃项)，0.0 表示无稀疏
//!
//! - **派生 Serialize + 手动 Deserialize**: 用于 mask_hash 计算(JSON 序列化后取 SHA-256)，
//!   反序列化时从 `active_ids` 重建 `active_set`，保持向后兼容
...
///
/// ```
/// use nexus_contracts::{SparseMask, ToolId};
///
/// let ids = vec![ToolId::new("tool-1"), ToolId::new("tool-2")];
/// let scores = vec![0.9, 0.1];
/// let mask = SparseMask::select_top_k(&ids, &scores, 1);
/// assert!(mask.is_active(&ToolId::new("tool-1")));
/// assert!(!mask.is_active(&ToolId::new("tool-2")));
/// ```
...
impl<T: Clone + PartialEq + Eq + Hash> SparseMask<T> {
    ...
    pub fn select_top_k(ids: &[T], scores: &[f32], k: usize) -> Self {
        ...
        // WHY 双重存储(Vec + HashSet): active_ids 提供有序遍历(O(K))，
