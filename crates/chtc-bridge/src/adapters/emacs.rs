//! Emacs 适配器 — 完整实现(execute 返回模拟执行结果)
//!
//! P2-4: 补全 execute 实现,对齐 VSCode 适配器的模拟执行模式。
//! Emacs 通过 S-expression 协议与 IDE 交互,execute 返回携带
...
impl IdeAdapter for EmacsAdapter {
    ...
    fn execute(&self, call: &UnifiedToolCall) -> Result<ToolCallResult, ChtcError> {
        ...
        // 预计 v3.x MCP Mesh 实装后替换为真实 Emacs S-expression 协议通信。
...
mod tests {
    ...
    #[test]
    fn test_emacs_adapter_execute_returns_success() {
        let a = EmacsAdapter::new();
