...
//! - **last_used_at 用 `RwLock<Instant>`**:读多写少(衰减循环读,路由路径写),
//!   通过内部可变性允许在 ExpertProfile 读锁下更新
...
/// - `last_used_at: RwLock<Instant>`:读多写少,衰减循环读、路由路径写
/// - 通过 `&ExpertProfile`(读锁)即可更新这两个字段,避免写锁争用
///
/// # 序列化注意
/// `AtomicU64` 和 `RwLock<Instant>` 不实现 Serialize/Deserialize。
