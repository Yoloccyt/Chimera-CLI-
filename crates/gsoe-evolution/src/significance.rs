...
//! `binomial_sf`(survival function)实现。N <= 32 时数值稳定
//! (与 `binomial_coefficient` 的 u64 溢出断言边界一致,R3-E06-4 订正:
//! 旧注释误写 N <= 10 与断言矛盾);典型 N = 3-5
//! (VETO_STREAK_THRESHOLD = 3),无需对数空间。
//!
//! ## 2. 单尾而非双尾
//!
//! 设计 §7.4 明确"连续 3 次显著回归"是单方向(回归方向)的统计显著性,
//! 单尾检验比双尾检验更符合语义。
//! ⚠️ R3-E06-4 订正(追溯 ADR-043 决策 3 的 R1 算术错误):
//! P(X >= 10 | n=14, p=0.5) = 1471/16384 ≈ 0.0898(8.98%),**非** 旧注释
//! 所写的 ≈0.059——10/14 实际未达 p<0.05,需 ≥11/14(P ≈ 0.0287)才达。
//! R2 晋级主判定已改用 Wilson 单侧下界(ADR-053-rev3),不受此误影响。
//!
...
///
/// WHY 0.05:经典统计学显著性水平(Fisher, 1925)。
/// ⚠️ R3-E06-4 订正:旧注释称与 ADR-043 的 71.4% 阈值"一致"且
/// P(X >= 10 | n=14) ≈ 0.059,实为 0.0898(8.98%)——10/14 未达 p<0.05,
/// ≥11/14 才达(P ≈ 0.0287)。R1 阈值的统计声明存在算术错误,
/// R2 已改用 Wilson 单侧下界主判定(ADR-053-rev3)。
...
/// # 算法选择
///
/// - **迭代乘法**:O(k) 时间复杂度,数值稳定(k <= n <= 32 时无溢出,
///   与下方 checked_mul 断言边界一致;R3-E06-4 订正旧注释的 n <= 10)
/// - **对数空间**:O(k) 时间,适合大 n,但 N <= 32 时无必要
///
/// # 边界
/// - k > n:返回 0(不可能)
/// - k = 0 或 k = n:返回 1
...
/// # 数值稳定性
///
/// N <= 32 时直接求和,PMF 各项和 = 1.0,无累积误差问题
/// (与 binomial_coefficient 溢出断言边界一致,R3-E06-4 订正)。
/// N > 32 时建议改用对数空间计算(本模块不需要,VETO_STREAK_THRESHOLD = 3)。
