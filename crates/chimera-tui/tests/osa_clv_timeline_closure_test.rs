...
fn render_to_string(app: &mut TuiApp, width: u16, height: u16) -> String {
    ...
    // 注意:逐 cell 收集时 CJK 宽字符的续格是真实空格(ratatui Cell::reset 语义),
    // 中文在结果中必然呈 "偏 置 =0.100" 形态 —— 对含相邻 CJK 的文本做 contains
    // 断言须先去空白(见下方 dense 匹配),不能直接匹配逻辑文本。
    buffer
        .content()
        .iter()
...
/// 调试输出截断:按**字符边界**回退。
///
/// WHY 不能直接 `&s[..n]`:渲染内容含 CJK 标签与 `│` 制表符(多字节),
/// 字节下标 n 落在多字节字符内部会 panic("end byte index is not a char
/// boundary",2026-09-09 回归实测随机触发);断言失败时的上下文打印必须
/// 边界安全。
...
async fn osa_clv_recall_sync_through_real_pipeline() {
    ...
    // 召回读数行(此前恒为 N/A)。WHY 去空白匹配:整屏 buffer 逐 cell 收集时,
    // CJK 宽字符续格是真实空格,"偏置=0.100" 在 buffer 文本中必然呈 "偏 置 =0.100",
    // 直接 contains 永不命中;去空白后按双语 OR 断言三读数(en bias= / zh 偏置=)。
    // 读数值本身已由上方 state 级断言独立覆盖,此处守护的是"渲染到屏"。
    let dense: String = content.chars().filter(|c| !c.is_whitespace()).collect();
    assert!(
        dense.contains("needle@8=0.800")
            && (dense.contains("bias=0.100") || dense.contains("偏置=0.100"))
            && (dense.contains("chain=0.900") || dense.contains("链路=0.900")),
