...
impl Panel for TaskManagerPanel {
    ...
    fn render(&mut self, state: &TuiState, area: Rect, buf: &mut Buffer) {
        ...
        // PS-2 批次3:象限 provider 在生产装配面**从未注册**(全仓仅 tests 桩),
        // 故 `quadrant_status_available()` 恒为 false —— 原 `if` 分支是死代码,
        // 且它读的是 L9 全局单例(越层依赖)。现移除 L10→L9 直调,直接渲染
        // 诚实标记(沿用 W8 假遥测治理既有文案 `panel.task.no_provider`)。
