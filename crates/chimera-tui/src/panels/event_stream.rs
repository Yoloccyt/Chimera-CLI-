...
mod tests {
    ...
    #[test]
    fn test_event_matches_keyword_by_request_id() {
        // P1 回执归属:`request_id` 进入事件可搜索文本是**契约行为**
        // (event_search_text 序列化整个事件),用户可据此在事件流中定位
        // 某一次具体派发;该断言把过滤面变化显式固化,防止无意回退。
        let event = NexusEvent::TuiActionRequested {
            metadata: EventMetadata::new("chimera-tui"),
            request_id: "tui-42".into(),
            action_id: "quest.pause".into(),
            payload: "{}".into(),
            source: event_bus::ActionSource::Palette,
        };
        assert!(
            event_matches_keyword(&event, "tui-42"),
            "按 request_id 过滤应命中"
        );
        assert!(
            event_matches_keyword(&event, "quest.pause"),
            "按 action_id 过滤应命中"
        );
        assert!(
            !event_matches_keyword(&event, "tui-43"),
            "其它 request_id 不应命中"
        );
    }
