...
impl Panel for RouterPanel {
    ...
    fn title(&self) -> Line<'static> {
        Line::from(crate::t!("panel.border.router"))
    }
...
mod tests {
    ...
    fn test_router_panel_content_default_state() {
        // 并行竞态防护:断言含中文标记 [低],全局 locale 被其它并行测试切换时随机失败
        // (2026-09-09 回归实测 RUN1 绿 / RUN2 红),拿测试锁并显式锁定 Zh。
        let _locale_guard = crate::i18n::locale_test_guard();
        crate::i18n::set_locale(crate::i18n::Locale::Zh);
    ...
    fn test_router_panel_content_low_hit_rate_marker() {
        ...
        // 并行竞态防护:断言含中文标记 [低],同 default_state 用例
        let _locale_guard = crate::i18n::locale_test_guard();
        crate::i18n::set_locale(crate::i18n::Locale::Zh);
        let mut state = TuiState::new();
        state.router_metrics = RouterMetrics {
    ...
    fn test_router_panel_content_empty_hot_capabilities() {
        ...
        // 并行竞态防护:断言含中文标记,同 default_state 用例
        let _locale_guard = crate::i18n::locale_test_guard();
        crate::i18n::set_locale(crate::i18n::Locale::Zh);
        let mut state = TuiState::new();
        state.router_metrics = RouterMetrics {
