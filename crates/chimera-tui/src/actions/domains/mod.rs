...
//!   `ActionDescriptor` 列表;`ActionRegistry::with_builtin_domains` 聚合五域。
//!   (P1,ADR-072:task.* 无 per-task 执行模型,is_core 空转入口已下线;
//!   task CRUD 真实化见 Phase 3 ADR,恢复时重新引入 task 域)
...
///
/// WHY 集中聚合:`ActionRegistry` 通过本函数一次性纳入五域,新增域只需
/// 在此追加一行,不改动注册表内部逻辑。
