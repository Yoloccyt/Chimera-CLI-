...
//!
//! 对应架构层:L3 Storage
//! 对应任务:Week 8 Task 1.1(WAL 崩溃恢复压测)+ C-02 修复回归
//!
//! # 验证目标
//! - 1000 次崩溃恢复循环,每次:写入 N 条 → 模拟崩溃(drop)→ 重开 → recover → 验证数据完整
//! - 测量单次崩溃恢复的端到端延迟(含 spawn_blocking 调度开销)
//! - 确保零数据丢失(已 commit 的 entry 不在 recover 列表,未 commit 的 entry 全部恢复)
//!
//! # C-02 修复适配
//! - SqliteWal 所有方法改为 async + spawn_blocking(避免阻塞 async runtime)
//! - 本基准在同步 criterion 闭包内通过 `tokio::runtime::Runtime::block_on` 调用 async API
//! - 测量值包含 spawn_blocking 线程池调度开销,反映真实生产延迟
//!
//! # 运行
//! ```bash
...
use scc_cache::wal::{SqliteWal, WalEntry, WalOperation};
...
///
/// # 参数
/// - `runtime`:tokio runtime(用于驱动 async SqliteWal 方法)
/// - `wal_path`:SQLite WAL 文件路径
/// - `num_entries`:每次写入的 entry 总数(其中前一半 commit,后一半不 commit)
///
/// # 返回
/// - `Duration`:recover() 调用的端到端延迟(含 spawn_blocking 调度)
///
/// # 断言
...
/// # WHY async 函数在 block_on 中调用:C-02 修复后 SqliteWal 方法为 async,
/// 同步 criterion 闭包需通过 runtime.block_on 驱动
fn single_crash_recovery_cycle(
    runtime: &Runtime,
    wal_path: &str,
    num_entries: usize,
) -> std::time::Duration {
    ...
    runtime.block_on(async {
        let wal = SqliteWal::new(wal_path.to_string())
            .await
            .expect("SqliteWal 创建应成功");
    ...
    // 阶段 2:重开同一文件,调用 recover(测量延迟,含 spawn_blocking 调度)
    let start = Instant::now();
    let recovered = runtime.block_on(async {
        let wal = SqliteWal::new(wal_path.to_string())
            .await
            .expect("SqliteWal 重开应成功");
        wal.recover().await.expect("recover 应成功")
    });
    let elapsed = start.elapsed();

    // 阶段 3:验证数据完整性(零丢失)
    let expected_uncommitted = num_entries - half;
    assert_eq!(
        recovered.len(),
...
/// # WHY runtime.block_on:C-02 修复后 SqliteWal 方法为 async,需 tokio runtime 驱动
fn verify_1000_crash_recoveries(runtime: &Runtime) {
    ...
    for cycle in 0..CYCLES {
        ...
        let elapsed = single_crash_recovery_cycle(runtime, path, ENTRIES_PER_CYCLE);

        // 性能断言:单次崩溃恢复应 < 100ms(含 spawn_blocking 调度开销)
        // WHY 100ms 阈值:留足余量,避免 CI 环境抖动导致误报
        // WHY 阈值未收紧:spawn_blocking 引入线程池调度开销,实际延迟比同步版略高
        assert!(
...
/// Criterion 基准:单次崩溃恢复延迟(含 spawn_blocking 调度)
///
/// # 流程
...
///
/// # WHY 复用 Runtime:避免每次 iter 都创建/销毁 runtime 的开销,
/// runtime 在 bench 函数入口创建一次,所有 block_on 复用
fn bench_crash_recovery(c: &mut Criterion) {
    // 创建 tokio runtime 一次,复用于所有 block_on 调用
    // WHY current_thread:基准测量单线程延迟,无需多线程调度开销
    // WHY enable_time:tokio::time::timeout 在回归测试中需要,bench 本身不用
    let runtime = Runtime::new().expect("创建 tokio runtime 失败");

    // 步骤 1:1000 次崩溃恢复正确性验证(对应任务要求)
    verify_1000_crash_recoveries(&runtime);
    ...
    c.bench_function("crash_recovery_single", |b| {
        b.iter_batched(
            ...
            |(path, _dir): (String, TempDir)| {
                ...
                let elapsed = single_crash_recovery_cycle(&runtime, &path, 10);
                black_box(elapsed);
            },
