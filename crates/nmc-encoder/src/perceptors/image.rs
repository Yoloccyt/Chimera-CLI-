...
fn image_statistical_embedding(bytes: &[u8], dim: usize) -> Vec<f32> {
    ...
    // WHY 归一化:所有直方图使用密度(比例)而非原始计数,
    // 使相同内容不同大小的图像产生方向一致的嵌入(cosine similarity 不受尺寸影响)。
    let total_bytes = bytes.len() as f64;

    // 阶段 1:RGB 颜色直方图(每 3 字节视为 RGB)
    let mut rgb_histogram = [0u32; 512]; // 8×8×8 = 512 bins
    for chunk in bytes.chunks(3) {
        let r = (chunk.first().copied().unwrap_or(0) as usize) / 32;
    ...
    for (i, &count) in brightness_histogram.iter().enumerate() {
        ...
        features[idx] += (count as f64 / total_bytes) * 0.5; // 亮度权重较低
    }

    // 阶段 3:边缘密度(相邻字节差分)
    let edge_total = (bytes.len() - 1).max(1) as f64;
    let mut edge_histogram = [0u32; 16];
    for i in 1..bytes.len() {
        // WHY unsigned_abs: i16::abs() 在 i16::MIN 时会溢出,unsigned_abs() 安全无溢出
        let diff = (bytes[i] as i16 - bytes[i - 1] as i16).unsigned_abs() as usize;
    ...
    for (i, &count) in edge_histogram.iter().enumerate() {
        ...
        features[idx] += (count as f64 / edge_total) * 0.3;
    }

    // 阶段 4:纹理特征(局部字节模式,类似 LBP)
    let texture_total = bytes.len().saturating_sub(2).max(1) as f64;
    let mut texture_histogram = [0u32; 256];
    for i in 1..bytes.len().saturating_sub(1) {
        let center = bytes[i];
        let mut pattern = 0u8;
        if i > 0 && bytes[i - 1] > center {
            pattern |= 1;
        }
        if i + 1 < bytes.len() && bytes[i + 1] > center {
            pattern |= 2;
        }
        // 每 8 个模式累积为一个字节索引
    ...
    for (i, &count) in texture_histogram.iter().enumerate() {
        ...
        features[idx] += (count as f64 / texture_total) * 0.2;
    }

    // 阶段 5:全局统计特征(已归一化,与图像尺寸无关)
    let mean = bytes.iter().map(|&b| b as f64).sum::<f64>() / total_bytes;
    let variance = bytes
        .iter()
        .map(|&b| {
...
mod tests {
    ...
    fn test_image_perceptor_different_data() {
        let p = ImagePerceptor::new();
        let elem1 = p
            .perceive(&PerceptionInput::Image(vec![0xFF; 1024]))
            .unwrap();
        let elem2 = p
            .perceive(&PerceptionInput::Image(vec![0x00; 1024]))
            .unwrap();
        // 全白 vs 全黑,应产生不同哈希和嵌入
    ...
    fn test_image_perceptor_different_sizes() {
        let p = ImagePerceptor::new();
        let elem1 = p
            .perceive(&PerceptionInput::Image(vec![0x80; 100]))
            .unwrap();
        let elem2 = p
            .perceive(&PerceptionInput::Image(vec![0x80; 10000]))
            .unwrap();
