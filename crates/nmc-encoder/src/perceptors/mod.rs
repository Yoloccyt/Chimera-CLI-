...
/// v3.0 将替换为 EmbeddingClient 的语义嵌入。
...
mod tests {
    ...
    fn test_all_perceptors_return_valid_embedding() {
        ...
        let text_elem = text_p
            .perceive(&PerceptionInput::Text("hello world".into()))
            .unwrap();
        let image_elem = image_p
            .perceive(&PerceptionInput::Image(vec![0x80; 1024]))
            .unwrap();
        let video_elem = video_p
            .perceive(&PerceptionInput::Video(vec![0x80; 8192]))
            .unwrap();
        let audio_elem = audio_p
            .perceive(&PerceptionInput::Audio(vec![0x80; 4096]))
            .unwrap();
        let desktop_elem = desktop_p
            .perceive(&PerceptionInput::Desktop(
        ...
        assert_eq!(
            text_elem.embedding_dim(),
            512,
            "TextPerceptor 嵌入维度应为 512"
        );
        assert_eq!(
            image_elem.embedding_dim(),
            512,
            "ImagePerceptor 嵌入维度应为 512"
        );
        assert_eq!(
            video_elem.embedding_dim(),
            512,
            "VideoPerceptor 嵌入维度应为 512"
        );
        assert_eq!(
            audio_elem.embedding_dim(),
            512,
            "AudioPerceptor 嵌入维度应为 512"
        );
        assert_eq!(
            desktop_elem.embedding_dim(),
            512,
            "DesktopPerceptor 嵌入维度应为 512"
        );
    ...
    fn test_all_perceptors_different_inputs_different_hashes() {
        ...
        let elem1 = text_p
            ...
            .unwrap();
        let elem2 = text_p
            .perceive(&PerceptionInput::Text("world".into()))
            .unwrap();
