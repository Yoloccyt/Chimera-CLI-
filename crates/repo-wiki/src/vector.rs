...
impl VectorIndex {
    ...
    pub fn with_sqlite(dim: usize, db_path: &Path) -> Result<Self, WikiError> {
        ...
        conn.execute(
            ...
            [],
        ).map_err(|e| WikiError::DatabaseError(e))?;
        ...
        let mut index = Self {
            dim,
            vectors: RwLock::new(HashMap::new()),
            sqlite_conn: RwLock::new(Some(conn)),
        };
    ...
    fn load_from_sqlite(&self) -> Result<(), WikiError> {
        let conn_guard = self.sqlite_conn.read().map_err(|e| {
            WikiError::VectorIndexError(format!("sqlite rwlock poisoned: {e}"))
        })?;
        ...
        for row in rows {
            let (entry_id, embedding) = row.map_err(|e| WikiError::DatabaseError(e))?;
            if embedding.len() == self.dim {
