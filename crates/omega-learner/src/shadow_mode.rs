...
///
/// ⚠️ R3-E06-4 订正（算术错误，保留阈值不改行为）：旧注释称
/// “14 天中 10 天以上胜率在 R1 无真实优势时约为 5.9%”，实为
/// P(X >= 10 | n=14, p=0.5) = 1471/16384 ≈ 8.98%——10/14 并未达到
/// p < 0.05 显著性，需 ≥11/14（P ≈ 2.87%）才达。本阈值为 R1 历史
/// 决策值（ADR-043）予以保留；R2 晋级主判定已改用“独立批次 +
/// Wilson 单侧 95% 下界 > 0.5”（ADR-053-rev3），不再依赖本阈值。
