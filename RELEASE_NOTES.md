# chimela CLI v1.5.8-omega Release Notes
...
chimela CLI v1.5.8-omega 是 v1.5.7-omega 的 GA 后演进版本。本次迭代聚焦**版本号一致性同步**与**发布工程文档对齐**,无功能性代码变更。所有用户侧入口、安装脚本、CI/CD 产物、packaging 模板与文档均已对齐到版本 `v1.5.8-omega`,并保留 `chimera`/`aether` 兼容别名,确保现有脚本与用户习惯无缝迁移。
...
- **版本引用一致性修复**: 修正 README、Dockerfile、安装脚本、packaging 与 CI 中残留的 `1.5.3`–`1.5.7` 版本引用,统一为 `1.5.8-omega`。
...
[docs/release/v1.5.8-omega_installation_guide.md](./docs/release/v1.5.8-omega_installation_guide.md)
...
- **Docker 入口**: 镜像内 binary 入口为 `chimela`,可直接运行 `docker run --rm ghcr.io/yoloccyt/chimera-cli:v1.5.8-omega --version`。
- **Release 产物命名**: 5 平台 binary 与 `.deb` 包均以 `chimela` 为前缀,例如 `chimela-windows-x86_64.exe`、`chimela-cli_1.5.8-omega_amd64.deb`。
...
- 修正 README 中 badge、安装示例、Docker 示例的版本号,统一为 `1.5.8-omega`。
- 修正 `install.ps1` 与 `install.sh` 的默认版本提示与示例,统一为 `v1.5.8-omega`。
- 修正 `packaging/homebrew/chimela-cli.rb`、`packaging/scoop/chimela-cli.json`、`packaging/apt/chimela-cli.control` 中的版本引用。
- 修正 `.github/workflows/release.yml` Release 正文中的 Homebrew/Scoop/APT 命令与安装指南链接,指向 `v1.5.8-omega_installation_guide.md`。
...
Homebrew / Scoop / APT 的 packaging 文件当前为模板,版本号已更新为 `1.5.8-omega`;外部仓库初始化后将自动具备分发能力。
...
| Debian / Ubuntu (amd64) | `chimela-cli_1.5.8-omega_amd64.deb` |
| 校验文件 | `checksums.txt` |
...
docker pull ghcr.io/yoloccyt/chimera-cli:v1.5.8-omega
```
...
感谢 NEXUS-OMEGA 团队对发布工程、QA 加固与文档一致性的持续投入。v1.5.8-omega 为后续外部包管理器仓库初始化与更广泛的用户分发奠定了统一的命名与版本基线。
