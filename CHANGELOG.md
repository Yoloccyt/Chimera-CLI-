...
- **acb-governor**(L8 Parliament):能力衰减流体控制,45 测试,`AcbGovernor::with_event_bus` 模式,发布 `AcbCapabilityAdjusted` 事件
- **auto-dpo**(L5 Knowledge):自动 Direct Preference Optimization 数据收集,38 测试,`AutoDpoCollector::with_event_bus` 模式,发布 `DpoSampleCollected` 事件
...
| `auto-dpo` | 10+ | DPO 数据收集 + DpoSampleCollected 事件 + EventBus 集成 |
| `chimera-tui` | 15+ | ratatui 4 布局 + 5 输入模式 + 键盘交互 + crossterm 0.28 适配 |
| `event-bus` | 1 | 2 个新事件类型(AcbCapabilityAdjusted / DpoSampleCollected) |
