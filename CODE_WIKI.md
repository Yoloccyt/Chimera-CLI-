...
- **已完成**:Week 1-8(L1-L10 全层覆盖,35 个 crate 全部实现,覆盖率 100%)
...
- **v1.4.0 进展**:P0 + P1 前置必做完成(2026-07-10),P2/P3/P4 条件触发未满足 — P0 repo-wiki 监控指标接入(prometheus-client gauge `wiki_entries_total`,insert/delete 后自动刷新,entries >= 800 时 WARN 预警,4 TDD 测试)、P1 SqliteHistoryStore 持久化实现(HistoryStore trait 迁移至 history 模块,SQLite + MessagePack rmp-serde 序列化 VecDeque<f32>,SELECT-merge-INSERT OR REPLACE UPSERT,默认 Memory 向后兼容 opt-in SQLite,9 TDD 测试 + bench memory 70.9ns vs sqlite 98.0µs)、P2 M1 向量索引评估(entries < 100, KNN p95 < 1ms,继续延后)、P3 M2 RL 路由评估(持久化已就绪,历史数据需积累)、P4 M3 配置热重载评估(无 daemon 模式,无用户请求,继续延后)
...
**已知错误提示**:`AETHER_NEXUS_OMEGA_从零搭建完全指南.md` 中"37 crates 骨架"数量错误,实际为 35 crate(以 `Cargo.toml` workspace.members 与本 Wiki §3.1 为权威)。
...
**统计**:已实现 35 个,骨架 0 个,覆盖率 100%(Week 8 Task 2 补齐 acb-governor / auto-dpo / chimera-tui)
...
#### online-learning
- **职责**:跨层在线学习框架,通过 `ParameterRegistry` 统一注册可学习参数,支持运行时参数自适应
- **关键类型**:`LearnableParameter`(可学习参数抽象)、`OnlineLearner`(反馈驱动更新器)、`GradientDescent`(梯度下降实现)、`ParameterRegistry`(全局参数注册表)、`FeedbackSignal`(反馈信号:成功/失败/延迟/资源消耗)
- **设计决策**:统一注册表避免各 crate 各自实现学习逻辑;反馈驱动更新(Quest 成功率/延迟/资源消耗触发);参数状态 JSON 序列化持久化;基于 DashMap 并发安全
- **依赖**:L1 同层依赖 nexus-core + event-bus
...
Week 8 是 NEXUS-OMEGA 项目的收尾阶段,完成性能调优、crate 补齐、安全测试、跨平台发布、文档完善五大任务,实现 35/35 crate 全覆盖(100%)、3002+ 测试全绿、性能指标全部达标。
...
2. 第三方 SIMD 库(wide/pulp)会破坏 `#![forbid(unsafe_code)]` 35/35 覆盖
...
补齐 3 个骨架 crate,实现 35/35 全覆盖(100%):
...
- **已实现**:35 个 crate(L1×3 + L2×3 + L3×3 + L4×3 + L5×3 + L6×4 + L7×4 + L8×3 + L9×3 + L10×5)
- **骨架**:0 个 crate
- **总计**:35 个 crate,覆盖率 100% ✅
...
> **维护者**:NEXUS-OMEGA 团队
> **状态**:8 周推进计划全部完成,35/35 crate 全覆盖,3002+ 测试全绿
