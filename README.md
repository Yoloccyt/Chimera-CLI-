...
```bash
# 指定版本
sh install.sh --version v1.5.2-omega
...
docker build -t chimera:1.5.2-omega .

# 查看版本
docker run --rm chimera:1.5.2-omega --version

# 运行任务
docker run --rm chimera:1.5.2-omega run "解释 OMEGA 四定律"

# 挂载配置目录(持久化 ~/.aether/omega.yaml)
docker run --rm -v "$HOME/.aether:/root/.aether" chimera:1.5.2-omega config show
```
